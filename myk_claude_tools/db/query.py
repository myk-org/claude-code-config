...
class ReviewDB:
    ...
    def get_dismissed_comments(self, owner: str, repo: str) -> list[dict[str, Any]]:
        ...
        - ``addressed`` comments are only included when their type is
          ``outside_diff_comment``.  This type lacks a GitHub review thread
          that can be resolved, so the database is the only mechanism to
          auto-skip them on subsequent fetches.  Normal inline thread comments
          rely on GitHub's ``isResolved`` filter in the GraphQL query, so
          including them here could incorrectly auto-skip a similar new
          finding in a different PR.
