...
```bash
uv run ~/.claude/commands/scripts/general/review_db.py query "SELECT path, line, body, reply FROM comments WHERE priority = 'HIGH' AND status = 'skipped' LIMIT 100" --json
```
